...
async function runFeishu(args, config) {
  ...
  const pending = all.filter((r) => {
    ...
    if (/\/video\//.test(r.link)) return false; // 已有链接（含用户手工填的）不再动
    // 已补全 = 终态（带链接的行已被上一行过滤，这里是防御性兜底）
    if (r.status === '已补全') return false;
    // 「需人工确认（未写链接）」不再当终态：匹配池随缓存持续增长（云端每
    // 30 分钟累积一次），当初只有低置信候选的行之后可能变成满分匹配，
    // 保持可重试让它自动升级为已补全；用户若手工填了链接则被上面拦住。
    // 「未找到」同理不再当终态跳过：嵌入页窗口只有最新 10 条、浏览器通道
    // 也可能只返回 10 条，历史行很容易被误判为未找到。账号级抓取本来每轮
    // 都要做，重试的边际成本接近 0，配合 mergeVideos 的历史缓存叠加，
    // 能自动把误判的行补回来。（--retry 参数保留兼容，现在恒为可重试）
    return true;
  });
